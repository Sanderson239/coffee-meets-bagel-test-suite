...
suite('GET /bagels route', () => {
  test('GET /bagels with correct authorization', (done) => {
    request(url)
    .get('/bagels/')
    ...
    .expect(200)
    .end((err, res) => {
      it('Res.body should be an array', () => {
      ...
      if (res.body.length >= 1) {
        it('A bagel should be an object with all the expected keys', () => {
          expect(res.body[0]).to.be.an('object').that.has.all.keys('id', 'name', 'age', 'gender', 'locations');
        })

        it('Bagel\'s locations property should be an array', () => {
          expect(res.body[0].locations).to.be.an('array');
          // important combine with length test below
        })

        it('Bagel\'s locations property should have at least one element', () => {
          expect(res.body[0].locations).to.have.lengthOf.above(0);
          // important combine with above
        })

        it('A location should be an object with all the expected keys', () => {
          expect(res.body[0].locations).to.be.an('object').that.has.all.keys('name', 'coordinates');
        })

        it('A location\'s coordinates should be an array of two existing coordinates', () => {
          expect(res.body[0].locations[0].coordinates).to.be.an('array').that.has.lengthOf.above(2);
          expect(res.body[0].locations[0].coordinates).to.have.lengthOf.above(2);
        })
      }
      done();
    });
...
suite('GET /bagels filtering by origin and distance', () => {
  test.only('All results, if any, should have at lease one location within the given distance of the origin when filtering by origin and distance', (done) => {
    request(url)
      ...
      .end((err, res) => {
          let withinDistance = res.body.some((ele, idx, arr) => {
            return !ele.activity.some((ele, idx, arr) => {return getDistance({lat: origin.lat, lng: origin.lng}, {lat: ele.coordinates[0], lng: ele.coordinates[1]}) <= dist})
          })
          ...
          // bagel.activity.forEach(location => {
            ...
            // }
          // })
          it('Result\'s should have locations all within the given distance of the origin', () => {
  ...
  test('All records returned should be at or above the minimum age', (done) => {
    request(url)
      ...
      .end((err, res) => {
        ...
        it('All results should be above the min_age', () => {
    ...
    request(url)
      ...
      .end((err, res) => {
        ...
        it('res.body should be an object with all the expected keys', () => {
          expect(res.body).to.be.an('object').that.has.all.keys('count', 'next', 'previous', 'results');
        })
        ...
        it('Res.body.results should only contain objects', () => {
          expect(res.body.results).should.all.be.an('object');
        })

        if (res.body.count.length >= 6) {
          ...
          it('res.body\'s previous property should be correct route', () => {
            expect(res.body.next).to.be.equal(`${url}/bagels/?limit=2&offset=2`);
          })
